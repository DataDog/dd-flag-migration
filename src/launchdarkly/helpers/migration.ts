...
/**
 * Remap allocation key prefixes from the original LD flag key to the resolved
 * Datadog flag key. This keeps allocation keys consistent with the flag key
 * when a prefix or custom key was applied during conflict resolution.
 *
 * Allocation keys are generated by `buildAllocations` using the original LD
 * `flag.key` (e.g. `my-flag-production-fallthrough`). When the flag is renamed
 * during migration (e.g. prefixed to `mobile-my-flag`), the allocation keys
 * must be updated to use the resolved Datadog key so that:
 *   1. Allocation keys match the flag key for traceability.
 *   2. Sync re-runs can match existing allocations by key (preserving UUIDs).
 *
 * Only the leading `sourceKey-` prefix is replaced; allocation keys that do
 * not start with `sourceKey-` are left unchanged.
 */
export function remapAllocationKeys(
	allocations: DatadogAllocationForFlagCreation[],
	sourceKey: string,
	datadogKey: string,
): DatadogAllocationForFlagCreation[] {
	if (datadogKey === sourceKey) return allocations;
	const prefix = `${sourceKey}-`;
	return allocations.map((a) =>
		a.key.startsWith(prefix)
			? { ...a, key: `${datadogKey}-${a.key.slice(prefix.length)}` }
			: a,
	);
}
 ...
 * tags for any Datadog teams granted editor access, and a tag identifying the
 * source LaunchDarkly project. The project tag is always added so that migrated
 * flags can be traced back to their LD origin. Duplicates are removed to keep
 * the tag list clean on re-migrations. Uses the project key (not the display
 * name) for a stable, machine-friendly identifier.
 */
export function buildFlagTags(
	...
	const teamTags = editorTeamHandles.map((handle) => `team:${handle}`);
	return [...new Set([...flagTags, ...teamTags, projectTag])];
}
