...
export function buildNonNegatedRules(
  ...
  if (segment.includedContexts.length > 0 || segment.excludedContexts.length > 0) {
  ...
  for (const rule of segment.rules) {
    ...
    for (const clause of rule.clauses) {
      ...
      conditions.push({ operator: mapped.operator, attribute: clause.attribute, value: mapped.values });
    }
  ...
  if (segment.included.length > 0) {
    groups.push({
      conditions: [{ operator: 'ONE_OF', attribute: 'key', value: segment.included }],
    });
  }
...
export function buildNegatedRules(
  ...
  if (segment.excluded.length > 0) {
    negatedRulesGroups.push({
      conditions: [{ operator: 'ONE_OF', attribute: 'key', value: segment.excluded }],
    });
  }
