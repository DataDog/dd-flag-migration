...
import type { DatadogEnvironment, DatadogCreateFlagRequest, DatadogCreatedFlag } from './types.js';
...
export async function validateDatadogKeys(apiKey: string, appKey: string): Promise<boolean> {
  try {
...
export async function fetchDatadogFlagKeys(
  ...
  while (true) {
    const response = await axios.get<{ data: JsonApiFlag[] }>(
      ...
      { headers: ddHeaders(apiKey, appKey), params: { limit, offset, is_archived: false } }
    );
...
export async function createFeatureFlag(
  ...
  const response = await axios.post<{ data: { id: string; attributes: { key: string } } }>(
    `${DD_BASE_URL}/api/unstable/feature-flags`,
    body,
