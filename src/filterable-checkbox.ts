...
const _filterableCheckbox = createPrompt(
  <T>(config: Config<T>, done: (value: T[]) => void) => {
    ...
    const [allItems, setAllItems] = useState<NormalizedChoice<T>[]>(() =>
      config.choices.map((c) => ({ name: c.name, value: c.value, checked: c.checked ?? false }))
    );
    ...
    const filteredItems = useMemo(() => {
      ...
      return allItems.filter((item) => stripAnsi(item.name).toLowerCase().includes(lower));
    }, [allItems, filterText]);
    ...
    useKeypress((key) => {
      ...
      if (isUpKey(key)) {
        ...
        if (target) {
          setAllItems(
            allItems.map((item) =>
              item.value === target.value ? { ...item, checked: !item.checked } : item
            )
          );
        }
        ...
        setAllItems(
          allItems.map((item) =>
            visibleValues.has(item.value) ? { ...item, checked: !allVisible } : item
          )
        );
        ...
        if (!key.ctrl && !meta && sequence && sequence.length === 1 && sequence.charCodeAt(0) >= 32) {
    ...
    const message = builtTheme.style.message(config.message, status === 'escaped' ? 'idle' : status);
    ...
    const filterLine = chalk.cyan('Filter: ') + (filterText ? chalk.bold(filterText) : chalk.dim('type to filter…')) + '  ' + countBadge;
    ...
    const helpTip = chalk.dim('↑↓/pgup/pgdn navigate  ·  space select  ·  ctrl+a select all  ·  esc back  ·  ⏎ confirm');

    const emptyMsg = filteredItems.length === 0 ? chalk.yellow('  No matches') : '';
...
export const filterableCheckbox = _filterableCheckbox as unknown as <T>(config: Config<T>) => Promise<T[] | null>;
