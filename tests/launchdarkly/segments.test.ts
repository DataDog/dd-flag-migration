...
import type { LDClause, LDFlag, LDRule, LDSegment } from '../../src/launchdarkly/types.js';
...
function makeRule(clauses: LDClause[], overrides: Partial<LDRule> = {}): LDRule {
  return {
    _id: 'r1',
    clauses,
    trackEvents: false,
...
function makeSegment(overrides: Partial<LDSegment> & { key: string }): LDSegment {
  return {
    name: overrides.key,
...
function makeFlag(key: string, envKey: string, clauses: LDClause[]): LDFlag {
  return {
    ...
    kind: 'boolean',
    key,
    variations: [{ _id: 'v0', value: true }, { _id: 'v1', value: false }],
...
describe('discoverSegmentRefs', () => {
  it('collects non-negated segmentMatch refs', () => {
    const flag = makeFlag('f1', 'prod', [makeClause({ op: 'segmentMatch', values: ['seg-a'], negate: false })]);
    const refs = discoverSegmentRefs([flag], ['prod']);
    expect(refs).toContainEqual({ segmentKey: 'seg-a', envKey: 'prod', negated: false });
  });

  it('collects negated segmentMatch refs', () => {
    const flag = makeFlag('f1', 'prod', [makeClause({ op: 'segmentMatch', values: ['seg-a'], negate: true })]);
    const refs = discoverSegmentRefs([flag], ['prod']);
    expect(refs).toContainEqual({ segmentKey: 'seg-a', envKey: 'prod', negated: true });
  });
  ...
  it('deduplicates identical refs', () => {
    ...
    const matches = refs.filter((r) => r.segmentKey === 'seg-a' && r.envKey === 'prod' && !r.negated);
    expect(matches).toHaveLength(1);
  });

  it('handles multi-value segmentMatch clause — one ref per value', () => {
    const flag = makeFlag('f1', 'prod', [makeClause({ op: 'segmentMatch', values: ['seg-a', 'seg-b'], negate: false })]);
    const refs = discoverSegmentRefs([flag], ['prod']);
    expect(refs).toContainEqual({ segmentKey: 'seg-a', envKey: 'prod', negated: false });
    expect(refs).toContainEqual({ segmentKey: 'seg-b', envKey: 'prod', negated: false });
  });

  it('only scans environments listed in envKeys', () => {
    const flag = makeFlag('f1', 'staging', [makeClause({ op: 'segmentMatch', values: ['seg-a'], negate: false })]);
    const refs = discoverSegmentRefs([flag], ['prod']);
    expect(refs).toHaveLength(0);
  });

  it('ignores non-segmentMatch clauses', () => {
    const flag = makeFlag('f1', 'prod', [makeClause({ op: 'in', attribute: 'country', values: ['US'] })]);
    const refs = discoverSegmentRefs([flag], ['prod']);
    expect(refs).toHaveLength(0);
  });
});
...
describe('getCreationType', () => {
  it('returns LIST for single rule, single key-in clause, no excluded', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'in', attribute: 'key', values: ['u1'] })])],
    });
  ...
  it('returns RULES for multi-rule segment', () => {
    const seg = makeSegment({
      ...
      rules: [
        ...
        makeRule([makeClause({ op: 'in', attribute: 'country', values: ['US'] })], { _id: 'r2' }),
      ],
    });
    expect(getCreationType(seg)).toBe('RULES');
  });
  ...
  it('returns RULES for a non-key attribute', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'in', attribute: 'country', values: ['US'] })])],
    });
    expect(getCreationType(seg)).toBe('RULES');
  });
...
describe('buildNonNegatedRules', () => {
  it('maps a single rule clause to one targeting rule', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'in', attribute: 'tenant', values: ['acme'] })])],
    });
  ...
  it('multi-rule segment → one targeting rule per rule (OR semantics)', () => {
    const seg = makeSegment({
      key: 's',
      rules: [
        makeRule([makeClause({ op: 'in', attribute: 'plan', values: ['beta'] })]),
        makeRule([makeClause({ op: 'in', attribute: 'role', values: ['qa'] })], { _id: 'r2' }),
      ],
    });
    const result = buildNonNegatedRules(seg);
    expect(result).not.toBeNull();
  ...
  it('excluded adds NOT_ONE_OF on key into every group', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'in', attribute: 'plan', values: ['pro'] })])],
      excluded: ['bad-user'],
    });
    const result = buildNonNegatedRules(seg);
    expect(result).not.toBeNull();
  ...
  it('rules + included + excluded: full formula (rules∨included)∧¬excluded', () => {
    ...
    for (const r of result!) {
      const notOneOf = r.conditions.find((c) => c.operator === 'NOT_ONE_OF' && c.attribute === 'key');
  ...
  it('returns null for nested segment (segmentMatch in segment rules)', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'segmentMatch', values: ['other-seg'] })])],
    });
    expect(buildNonNegatedRules(seg)).toBeNull();
  });
...
describe('buildNegatedRules', () => {
    ...
    const seg = makeSegment({
      ...
      rules: [
        makeRule([
          ...
          makeClause({ _id: 'c2', op: 'in', attribute: 'plan', values: ['beta'] }),
        ]),
        makeRule([
          makeClause({ _id: 'c3', op: 'in', attribute: 'role', values: ['qa'] }),
          makeClause({ _id: 'c4', op: 'in', attribute: 'org_id', values: ['1', '2'] }),
        ], { _id: 'r2' }),
      ],
    });
    const result = buildNegatedRules(seg);
  ...
  it('included-only + excluded: (¬included) ∨ excluded → two groups', () => {
    ...
    const hasNotOneOf = result!.some(
      (r) => r.conditions[0].operator === 'NOT_ONE_OF' && r.conditions[0].value!.includes('u1'),
    );
    const hasOneOf = result!.some(
      (r) => r.conditions[0].operator === 'ONE_OF' && r.conditions[0].value!.includes('u-bad'),
    );
  ...
  it('returns null for explosion guard exceeded', () => {
    const fiveClauses = Array.from({ length: 5 }, (_, i) =>
      makeClause({ _id: `c${i}`, attribute: `a${i}`, op: 'in', values: [`v${i}`] }),
    );
    const seg = makeSegment({
      key: 's',
      rules: Array.from({ length: 5 }, (_, i) => makeRule(fiveClauses, { _id: `r${i}` })),
    });
  ...
  it('returns null for nested segment in rules', () => {
    const seg = makeSegment({
      key: 's',
      rules: [makeRule([makeClause({ op: 'segmentMatch', values: ['other'] })])],
    });
    expect(buildNegatedRules(seg)).toBeNull();
  });
