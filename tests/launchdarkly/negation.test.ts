...
import type { DatadogCondition, DatadogTargetingRule } from '../../src/types.js';

// ─── negateCondition ──────────────────────────────────────────────────────────

describe('negateCondition', () => {
  it('negates ONE_OF to NOT_ONE_OF', () => {
    const c: DatadogCondition = { operator: 'ONE_OF', attribute: 'key', value: ['a'] };
    expect(negateCondition(c)).toEqual({ operator: 'NOT_ONE_OF', attribute: 'key', value: ['a'] });
  });

  it('negates NOT_ONE_OF to ONE_OF', () => {
    const c: DatadogCondition = { operator: 'NOT_ONE_OF', attribute: 'key', value: ['b'] };
    expect(negateCondition(c)).toEqual({ operator: 'ONE_OF', attribute: 'key', value: ['b'] });
  });

  it('negates MATCHES to NOT_MATCHES', () => {
    const c: DatadogCondition = { operator: 'MATCHES', attribute: 'email', value: ['.*@dd.com'] };
    expect(negateCondition(c)).toEqual({ operator: 'NOT_MATCHES', attribute: 'email', value: ['.*@dd.com'] });
  });

  it('negates LT to GTE', () => {
    const c: DatadogCondition = { operator: 'LT', attribute: 'age', value: ['18'] };
    expect(negateCondition(c)).toEqual({ operator: 'GTE', attribute: 'age', value: ['18'] });
  });

  it('negates LTE to GT', () => {
    const c: DatadogCondition = { operator: 'LTE', attribute: 'age', value: ['18'] };
    expect(negateCondition(c)).toEqual({ operator: 'GT', attribute: 'age', value: ['18'] });
  });

  it('negates GT to LTE', () => {
    const c: DatadogCondition = { operator: 'GT', attribute: 'age', value: ['18'] };
    expect(negateCondition(c)).toEqual({ operator: 'LTE', attribute: 'age', value: ['18'] });
  });

  it('negates GTE to LT', () => {
    const c: DatadogCondition = { operator: 'GTE', attribute: 'age', value: ['18'] };
    expect(negateCondition(c)).toEqual({ operator: 'LT', attribute: 'age', value: ['18'] });
  });

  it('negates SEMVER_LT to SEMVER_GTE', () => {
    const c: DatadogCondition = { operator: 'SEMVER_LT', attribute: 'v', value: ['2.0.0'] };
    expect(negateCondition(c)).toEqual({ operator: 'SEMVER_GTE', attribute: 'v', value: ['2.0.0'] });
  });

  it('negates SEMVER_EQ to SEMVER_NEQ', () => {
    const c: DatadogCondition = { operator: 'SEMVER_EQ', attribute: 'v', value: ['1.0.0'] };
    expect(negateCondition(c)).toEqual({ operator: 'SEMVER_NEQ', attribute: 'v', value: ['1.0.0'] });
  });
  ...
  it('returns null for unknown operator', () => {
    const c: DatadogCondition = { operator: 'UNKNOWN_OP', attribute: 'x', value: ['y'] };
    expect(negateCondition(c)).toBeNull();
  });
});
...
describe('cartesianProduct', () => {
  ...
  it('wraps a single one-element group into one targeting rule', () => {
    const c: DatadogCondition = { operator: 'ONE_OF', attribute: 'a', value: ['1'] };
  ...
  it('produces 2×2=4 rules for two groups of 2', () => {
    const c1: DatadogCondition = { operator: 'ONE_OF', attribute: 'a', value: ['1'] };
    const c2: DatadogCondition = { operator: 'ONE_OF', attribute: 'a', value: ['2'] };
    const c3: DatadogCondition = { operator: 'ONE_OF', attribute: 'b', value: ['x'] };
    const c4: DatadogCondition = { operator: 'ONE_OF', attribute: 'b', value: ['y'] };
    const result = cartesianProduct([[c1, c2], [c3, c4]]);
    expect(result).toHaveLength(4);
    for (const r of result) expect(r.conditions).toHaveLength(2);
    const combos = result.map((r) => `${r.conditions[0].value![0]},${r.conditions[1].value![0]}`);
...
describe('negateTargetingRules', () => {
  ...
  it('negates a single rule with one condition', () => {
    ...
    expect(negateTargetingRules(rules)).toEqual([
      { conditions: [{ operator: 'NOT_ONE_OF', attribute: 'country', value: ['US'] }] },
    ]);
  });
  ...
  it('returns null when the product exceeds 100 groups (explosion guard)', () => {
    ...
    const fiveClauses: DatadogCondition[] = Array.from({ length: 5 }, (_, i) => ({
      operator: 'ONE_OF',
      attribute: `attr${i}`,
      value: [`val${i}`],
    }));
  ...
  it('allows exactly 100 groups (boundary — should not be null)', () => {
    ...
    const tenClauses: DatadogCondition[] = Array.from({ length: 10 }, (_, i) => ({
      operator: 'ONE_OF',
      attribute: `attr${i}`,
      value: [`val${i}`],
    }));
